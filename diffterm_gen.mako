...
__global__ void gpu_diffterm_lv1_kernel(
	...
	__shared__ double s_q[s_qend][${shared_dim_y}][${shared_dim_x}];
	...
	tidx = threadIdx.x;
	tidz = threadIdx.y;
	while(tidz <  ${shared_dim_y} && si < g->dim_g[0] && sj < g->dim_g[1] && sk < g->dim_g[2]){
...
#undef 	q
	...
	__syncthreads();
	for(sj = blockIdx.y*blockDim.y+threadIdx.y, tidy=threadIdx.y; tidy<${shared_dim_y}; sj+=blockDim.y, tidy+=blockDim.y){
		for(si = blockIdx.x*blockDim.x+threadIdx.x, tidx=threadIdx.x; tidx<${shared_dim_x}; si+=blockDim.x, tidx+=blockDim.x){
...
__global__ void gpu_diffterm_lv2_kernel(
	...
	__shared__ double ux[${shared_dim_y}][${shared_dim_x}];
	__shared__ double wz[${shared_dim_y}][${shared_dim_x}];
	__shared__ double vy[${shared_dim_y}][${shared_dim_x}];
...
void gpu_diffterm_lv1_lv2(
	...
	dim3 grid_dim(CEIL(h_const.dim_g[0], BLOCK_DIM_X), CEIL(h_const.dim_g[1], BLOCK_DIM_Y), h_const.dim_g[2]);
