...
for block_dim_x in block_dim_x_choices:
	for block_dim_y in block_dim_y_choices:
				...
				for shared_pad in pad_choices:
					for smem in smem_choices:
						for bypass_l1 in bypass_l1_choices:
							if smem == 16 and 4*ceiling((block_dim_x+ng+ng)*8, shared_pad)*(block_dim_y+ng+ng) > 16*1024:
								continue;
							elif smem == 48 and 4*ceiling((block_dim_x+ng+ng)*8, shared_pad)*(block_dim_y+ng+ng) > 48*1024:
								continue;
